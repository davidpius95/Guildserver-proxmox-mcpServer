...
# Tailscale SSH must be on for keyless remote access via tailnet identity.
# Idempotent, and a no-op when already set (does not trigger re-auth).
tailscale set --ssh=true 2>/dev/null || true
